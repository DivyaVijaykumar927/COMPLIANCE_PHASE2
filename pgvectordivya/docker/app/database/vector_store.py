...
class VectorStore:
    ...
    def __init__(self):
        """Initialize the VectorStore with settings, Hugging Face model, and Timescale Vector client."""
        self.settings = get_settings()
        self.embedding_model_name = self.settings.huggingface.model_name
        self.vector_settings = self.settings.vector_store
    ...
    def get_embedding(self, text: str) -> List[float]:
        """
        Generate embedding for the given text using Hugging Face models.
        ...
        # Use the [CLS] token's embedding (first hidden state) as the representation
